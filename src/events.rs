...
use crate::storage_types::{StakeConfig, TransferFeeConfig, WrapState};

/// All events emitted by the contract.
///
/// Note: soroban-sdk's `#[contracttype]` only supports unit and tuple
/// variants, so every payload is a tuple (named fields are rejected).
#[contracttype]
...
pub enum Event {
    ...
    Transfer(Address, Address, u64, TransferFeeConfig),
}
